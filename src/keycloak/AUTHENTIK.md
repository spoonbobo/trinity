...
Set these env vars (in `src/.env` or your deployment environment):
...
```bash
docker compose -f src/docker-compose.yml up -d --build keycloak keycloak-idp-bootstrap supabase-auth
```
